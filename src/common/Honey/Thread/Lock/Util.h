...
namespace priv
{
    ...
    template<class Locks, int... Seq>
    void lock(Locks&& locks, mt::IntSeq<Seq...>)
    {
...
void lock(Locks&&... locks)                     { priv::lock(forward_as_tuple(forward<Locks>(locks)...), mt::IntSeqGen<sizeof...(Locks)>()); }
